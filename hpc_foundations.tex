...
% Font encoding (fixes rendering in pdfpc/poppler)
...
% Set to true for presentation (blurs some slides), false for full content
% =============================================================================
\newif\ifpresentationmode
\presentationmodetrue  % <-- Change to \presentationmodefalse for notes/full mode

% Blur overlay command (frosted glass effect over slide content, keeps title visible)
\newcommand{\bluroverlay}{%
  \ifpresentationmode
    \begin{tikzpicture}[remember picture, overlay]
      \fill[white, opacity=0.85]
        ([yshift=-1.3cm]current page.north west) rectangle (current page.south east);
      \node[opacity=0.5] at (current page.center) {\Large\textit{Available in extended version}};
    \end{tikzpicture}
  \fi
}
...
\newcommand{\quiztime}{%
  \vfill
  \centering
  {\Huge \textbf{Quiz Time!}}
  \vskip1.5em
  {\Large Understanding Parallelism}
  \vskip2em
  \ifpresentationmode
    {\large \href{https://fuiz.us/create}{\beamergotobutton{Start Quiz}}}
  \else
    {\large \href{https://fuiz.us/play}{\beamergotobutton{Start Quiz}}}
  \fi
  \vfill
}
...
\author{\emph{Flavio Calvo} \\ Université de Lausanne}
...
\begin{frame}{Course material and tools}
    \vspace{-1em}
	\begin{itemize}
		\item \textbf{Wi-Fi}:
		\begin{itemize}
            \scriptsize
			\item SSID: \textbf{eduroam}
			\item CHUV and external collaborators: \url{https://crypto.unil.ch} (VPN)
			\item Still not working? Try SSID: \textbf{DCSRNET} \& PWD: \textbf{DCSR\_DCSR}
		\end{itemize}
		\item \textbf{Interactive Pad}: {\scriptsize \url{https://ood.dcsr.unil.ch/public/ipc-pad}}
		\begin{itemize}
            \scriptsize
			...
			\item Feedback
		\end{itemize}
		\item \textbf{Open OnDemand}: {\scriptsize \url{https://ood.dcsr.unil.ch/}}
		\begin{itemize}
            \scriptsize
			\item Contains the Parallel HPC Course App
			\item Slides available in the App
			\item Hands-on in the App
		\end{itemize}
		\item \textbf{Quiz}: {\scriptsize \url{https://fuiz.org/play}}
	\end{itemize}
\end{frame}
...
\begin{frame}{What is Parallel Computing?}
    \begin{itemize}
        ...
        \end{itemize}
        \item \textbf{Concurrency is not Parallelism!}
        \begin{itemize}
            \item \textit{Concurrency}: managing multiple tasks at once (may interleave on one core)
            \item \textit{Parallelism}: executing multiple tasks simultaneously (requires multiple cores)
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Parallelism vs Concurrency}
	...
	\begin{columns}[T]
		\begin{column}{0.5\textwidth}
			\textbf{Concurrency:} Tasks that \emph{appear} to happen simultaneously but don't actually overlap
			\vskip1em
			\textbf{Example: Biking while texting on phone}
		...
		\begin{column}{0.5\textwidth}
			...
			\vskip1em
			\textbf{Example: Biking while talking on phone}
			\vskip.2em
			\begin{tikzpicture}[scale=0.85]
				% Time axis
				\draw[->, thick] (0,0) -- (6,0) node[right] {Time};
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
			...
			\begin{tikzpicture}[scale=0.8]
				...
				\draw[<-, thick] (-0.4, 0.2) -- (-0.4, 6.3) node[midway, left, align=right] {\small Capacity};
			\end{tikzpicture}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
			...
			\begin{tikzpicture}[scale=0.8]
				...
				\node at (2, 5.6) {\tiny Accelerators (GPU/FPGA)};
				\node at (2, 4.9) {\tiny Processes (MPI)};
				\node at (2, 4.2) {\tiny Threads (OpenMP)};
				\node at (2, 3.5) {\tiny SIMD (Vector.)};
				\node at (2, 2.8) {\tiny ILP (Hardw.)};
...
\begin{frame}{Race Conditions \& Thread Safety}
	\vspace{-1em}
	\footnotesize
	\textbf{What is a race condition?}
	When multiple threads access shared data simultaneously and at least one modifies it, the outcome depends on timing---making results \textbf{unpredictable}.
	\vskip0.5em
	\begin{columns}[T]
		...
		\begin{column}{0.55\textwidth}
			\textbf{What goes wrong---interleaved execution:}
			\begin{center}
			\scriptsize
			\begin{tabular}{c|l|l|c}
			\textbf{Time} & \textbf{Thread A} & \textbf{Thread B} & \textbf{counter} \\
			\hline
			1 & read (0) & & 0 \\
			2 & & read (0) & 0 \\
			3 & compute 0+1 & & 0 \\
			4 & & compute 0+1 & 0 \\
			5 & write 1 & & 1 \\
			6 & & write 1 & \textcolor{red}{1} \\
			\end{tabular}
			\end{center}
			\vskip0.3em
			\textcolor{red}{Lost update!} Both incremented, but counter is 1, not 2.
		\end{column}
	\end{columns}
	\vskip0.5em
	\textbf{Thread-safe code:} Code that functions correctly when accessed by multiple threads simultaneously, without race conditions or data corruption.
\end{frame}

\begin{frame}{Synchronization Primitives}
	\vspace{-1em}
	\scriptsize
	\textbf{Tools to prevent race conditions:}
	\vskip0.5em
	\begin{center}
	\begin{tabular}{|l|p{4.5cm}|p{4cm}|}
		\hline
		\textbf{Primitive} & \textbf{Description} & \textbf{Use case} \\
		\hline
		\textbf{Mutex (Lock)} & Only one thread can hold it; same thread locks and unlocks & Protecting shared data structures \\
		\hline
		\textbf{Semaphore} & Signaling mechanism; one thread can wake another waiting on it & Producer-consumer, resource pools \\
		\hline
		\textbf{Atomic ops} & Hardware-guaranteed indivisible read-modify-write & Simple counters, flags \\
		\hline
		\textbf{Critical section} & Code region executed by one thread at a time & Short protected blocks \\
		\hline
		\textbf{Barrier} & All threads wait until everyone reaches this point & Synchronizing phases \\
		\hline
	\end{tabular}
	\end{center}
	\vskip0.5em
	\textbf{Semaphores in detail:}
	\begin{itemize}
		\item \texttt{wait()}: block until signaled, then proceed
		\item \texttt{signal()}: wake up a waiting thread
		\item Unlike mutexes, the signaling thread can differ from the waiting thread
	\end{itemize}
	\vskip0.3em
	\textbf{Trade-off:} More synchronization = correctness, but also less parallelism.
\end{frame}

\begin{frame}{Deadlocks}
	\vspace{-1em}
	\scriptsize
	\textbf{What is a deadlock?}
	Two or more threads stuck forever, each waiting for a resource held by another.
	\vskip0.1em
	\begin{columns}[T]
		\begin{column}{0.5\textwidth}
			\textbf{Classic example (pseudocode):}
			\begin{center}
			\texttt{\scriptsize
			\begin{tabular}{l|l}
			\textbf{Thread A} & \textbf{Thread B} \\
			\hline
			lock(resource\_1) & lock(resource\_2) \\
			lock(resource\_2) & lock(resource\_1) \\
			\quad $\uparrow$ waits & \quad $\uparrow$ waits \\
			... & ... \\
			\end{tabular}
			}
			\end{center}
		...
		\begin{column}{0.5\textwidth}
			...
			\centering
			\begin{tikzpicture}[scale=0.8]
				\node[draw, rounded corners, fill=green!20, minimum width=1.5cm] (A) at (0, 2) {Thread A};
				\node[draw, rounded corners, fill=green!20, minimum width=1.5cm] (B) at (4, 2) {Thread B};
				\node[draw, fill=blue!20, minimum width=1.5cm] (R1) at (0, 0) {Resource 1};
				\node[draw, fill=blue!20, minimum width=1.5cm] (R2) at (4, 0) {Resource 2};
				\draw[->, thick] (A) -- (R1) node[midway, left] {\tiny holds};
				\draw[->, thick] (B) -- (R2) node[midway, right] {\tiny holds};
				\draw[->, thick, dashed, red] (A) -- (R2) node[midway, above] {\tiny wants};
				\draw[->, thick, dashed, red] (B) -- (R1) node[midway, above] {\tiny wants};
			\end{tikzpicture}
		\end{column}
	\end{columns}
	\vskip0.5em
	\textbf{Four conditions for deadlock} (all must be present):
	\begin{enumerate}
		\item \textbf{Mutual exclusion}---resources can't be shared
		\item \textbf{Hold and wait}---holding one resource while waiting for another
		\item \textbf{No preemption}---resources can't be forcibly taken
		\item \textbf{Circular wait}---A waits for B, B waits for A
	\end{enumerate}
	\vskip0.3em
	\textbf{Prevention:} Lock ordering $\cdot$ Timeouts $\cdot$ Lock-free algorithms
\end{frame}
...
\begin{frame}{Parallel Patterns: The Big Picture}
    \vspace{-1em}
	\footnotesize
	\textbf{Key insight:} Most parallel problems fit one of these communication patterns:
	\vskip0.5em
	\begin{center}
	\begin{tabular}{|l|c|l|l|}
		\hline
		\textbf{Pattern} & \textbf{Communication} & \textbf{Examples} & \textbf{Strategy} \\
		\hline
		Embarrassingly & \multirow{2}{*}{None} & Monte Carlo, rendering, & Simple \\
		Parallel & & parameter sweeps & distribution \\
		\hline
		Reduction & All $\to$ One & Sum, max, dot product & Tree reduction \\
		\hline
		\multirow{2}{*}{Stencil} & \multirow{2}{*}{Neighbors} & PDEs, image filters, & Domain decomp. \\
		& & simulations & + halo exchange \\
		\hline
		All-to-All & Everyone $\leftrightarrow$ All & FFT, sorting, dense LA & Collectives \\
		\hline
		Irregular & Data-dependent & Graphs, adaptive mesh & Task-based \\
		\hline
	\end{tabular}
	\end{center}
	\vskip0.5em
	\textbf{Your first question:} What's the communication pattern of my problem?
\end{frame}

\begin{frame}{Common Parallel Patterns}
	\vspace{-1em}
	\begin{columns}[T]
		\begin{column}{0.33\textwidth}
			...
			\begin{tikzpicture}[scale=0.6]
				% Input array
				\foreach \i in {0,...,3} {
					\draw[fill=blue!20] (\i*0.7, 1.5) rectangle (\i*0.7+0.5, 2);
					\node at (\i*0.7+0.25, 1.75) {\tiny \i};
				}
				% Arrows
				\foreach \i in {0,...,3} {
					\draw[->, thick] (\i*0.7+0.25, 1.4) -- (\i*0.7+0.25, 0.6);
					\node[right] at (\i*0.7+0.3, 1.0) {\tiny f};
				}
				% Output array
				\foreach \i in {0,...,3} {
					\draw[fill=green!20] (\i*0.7, 0) rectangle (\i*0.7+0.5, 0.5);
				}
			\end{tikzpicture}
			\\[0.3em]
			\scriptsize Same operation\\on each element\\[0.3em]
			\tiny\textit{Julia set, ray tracing}
		\end{column}
		\begin{column}{0.33\textwidth}
			\centering
			\textbf{Reduce}\\[0.3em]
			\begin{tikzpicture}[scale=0.6]
				% Input array
				\foreach \i in {0,...,3} {
					\draw[fill=blue!20] (\i*0.7, 2) rectangle (\i*0.7+0.5, 2.5);
				}
				% Tree reduction
				\draw[->, thick] (0.25, 1.9) -- (0.6, 1.4);
				\draw[->, thick] (0.95, 1.9) -- (0.6, 1.4);
				\draw[->, thick] (1.65, 1.9) -- (2.0, 1.4);
				\draw[->, thick] (2.35, 1.9) -- (2.0, 1.4);
				\draw[fill=yellow!30] (0.35, 1.0) rectangle (0.85, 1.4);
				\draw[fill=yellow!30] (1.75, 1.0) rectangle (2.25, 1.4);
				\draw[->, thick] (0.6, 0.9) -- (1.1, 0.4);
				\draw[->, thick] (2.0, 0.9) -- (1.5, 0.4);
				% Output
				\draw[fill=green!30] (1.05, 0) rectangle (1.55, 0.4);
			\end{tikzpicture}
			\\[0.3em]
			\scriptsize Combine all into\\one result\\[0.3em]
			\tiny\textit{Sum, max, histogram}
		\end{column}
		\begin{column}{0.33\textwidth}
			\centering
			\textbf{Stencil}\\[0.3em]
			\begin{tikzpicture}[scale=0.5]
				...
				\foreach \i in {0,...,4} {
					\foreach \j in {0,...,4} {
						\draw[fill=gray!10] (\i*0.5, \j*0.5) rectangle (\i*0.5+0.45, \j*0.5+0.45);
					}
				}
				% Highlight center and neighbors
				\draw[fill=red!30] (1.0, 1.0) rectangle (1.45, 1.45);
				\draw[fill=orange!30] (1.0, 0.5) rectangle (1.45, 0.95);
				\draw[fill=orange!30] (1.0, 1.5) rectangle (1.45, 1.95);
				\draw[fill=orange!30] (0.5, 1.0) rectangle (0.95, 1.45);
				\draw[fill=orange!30] (1.5, 1.0) rectangle (1.95, 1.45);
				% Arrow
				\draw[->, thick] (2.7, 1.2) -- (3.3, 1.2);
				% Result
				\draw[fill=green!30] (3.5, 1.0) rectangle (3.95, 1.45);
			\end{tikzpicture}
			\\[0.3em]
			\scriptsize Each element uses\\its neighbors\\[0.3em]
			\tiny\textit{Heat eq., blur, CFD}
		\end{column}
	\end{columns}
	\vskip1em
	\begin{columns}[T]
		...
		\begin{column}{0.4\textwidth}
			\centering
			\textbf{Map-Reduce}\\[0.3em]
			\begin{tikzpicture}[scale=0.55]
				% Data
				\draw[fill=blue!20] (0, 1.5) rectangle (2.5, 2);
				\node at (1.25, 1.75) {\tiny Data};
				% Map phase
				\draw[->, thick] (0.4, 1.4) -- (0.4, 0.9);
				\draw[->, thick] (1.25, 1.4) -- (1.25, 0.9);
				\draw[->, thick] (2.1, 1.4) -- (2.1, 0.9);
				\draw[fill=yellow!30] (0.1, 0.5) rectangle (0.7, 0.85);
				\draw[fill=yellow!30] (0.95, 0.5) rectangle (1.55, 0.85);
				\draw[fill=yellow!30] (1.8, 0.5) rectangle (2.4, 0.85);
				\node[right] at (2.5, 0.67) {\tiny Map};
				% Reduce
				\draw[->, thick] (0.4, 0.4) -- (1.1, 0.1);
				\draw[->, thick] (1.25, 0.4) -- (1.25, 0.1);
				\draw[->, thick] (2.1, 0.4) -- (1.4, 0.1);
				\draw[fill=green!30] (0.95, -0.3) rectangle (1.55, 0.05);
				\node[right] at (1.6, -0.12) {\tiny Result};
			\end{tikzpicture}
			\\[0.2em]
			\scriptsize Process locally, then combine\\
			\tiny\textit{Word count, statistics, ML}
		\end{column}
		\begin{column}{0.4\textwidth}
			\centering
			\textbf{Pipeline}\\[0.3em]
			\begin{tikzpicture}[scale=0.45]
				% Time labels on left
				\node[left, font=\tiny] at (-0.3, 2.4) {t=1};
				\node[left, font=\tiny] at (-0.3, 1.6) {t=2};
				\node[left, font=\tiny] at (-0.3, 0.8) {t=3};
				\node[left, font=\tiny] at (-0.3, 0.0) {t=4};
				% Stage labels on top
				\node[above, font=\tiny] at (0.4, 2.9) {S1};
				\node[above, font=\tiny] at (1.4, 2.9) {S2};
				\node[above, font=\tiny] at (2.4, 2.9) {S3};
				% t=1: only S1 works on item a
				\draw[fill=blue!30] (0, 2.2) rectangle (0.8, 2.7);
				\node at (0.4, 2.45) {\tiny a};
				% t=2: S1 works on b, S2 works on a
				\draw[fill=blue!30] (0, 1.4) rectangle (0.8, 1.9);
				\node at (0.4, 1.65) {\tiny b};
				\draw[fill=yellow!30] (1.0, 1.4) rectangle (1.8, 1.9);
				\node at (1.4, 1.65) {\tiny a};
				% t=3: S1 works on c, S2 works on b, S3 works on a
				\draw[fill=blue!30] (0, 0.6) rectangle (0.8, 1.1);
				\node at (0.4, 0.85) {\tiny c};
				\draw[fill=yellow!30] (1.0, 0.6) rectangle (1.8, 1.1);
				\node at (1.4, 0.85) {\tiny b};
				\draw[fill=green!30] (2.0, 0.6) rectangle (2.8, 1.1);
				\node at (2.4, 0.85) {\tiny a};
				% t=4: S2 works on c, S3 works on b
				\draw[fill=yellow!30] (1.0, -0.2) rectangle (1.8, 0.3);
				\node at (1.4, 0.05) {\tiny c};
				\draw[fill=green!30] (2.0, -0.2) rectangle (2.8, 0.3);
				\node at (2.4, 0.05) {\tiny b};
			\end{tikzpicture}
			\\[0.2em]
			\scriptsize 3 items in 5 steps (not 9)\\
			\tiny\textit{Video processing, compilers}
		\end{column}
		\begin{column}{0.1\textwidth}
		\end{column}
	\end{columns}
...
\begin{frame}{Choosing Your Parallelization Strategy}
	\vspace{-1em}
	\footnotesize
	\begin{center}
	\begin{tikzpicture}[
		decision/.style={signal, signal to=east and west, draw, fill=blue!10, text width=1.9cm, align=center, inner sep=2pt, font=\tiny, minimum height=0.7cm},
		result/.style={rectangle, draw, fill=green!20, text width=1.3cm, align=center, rounded corners, font=\tiny},
		example/.style={font=\tiny\itshape, text width=1.3cm, align=center},
		arrow/.style={->, thick, >=stealth},
		scale=0.85
	]
		% Root
		\node[decision] (root) at (0, 0) {Known loop bounds?};

		% Left branch - for-like (loops)
		\node[decision] (indep) at (-3.5, -1.4) {Iterations independent?};

		% Right branch - while-like (not loops)
		\node[decision] (recur) at (3.5, -1.4) {Recursive or irregular?};

		% Left-left: independent iterations
		\node[decision] (uniform) at (-5.5, -2.8) {Uniform work?};

		% Left-right: dependencies
		\node[decision] (deptype) at (-1.5, -2.8) {Dependency type?};

		% Results
		\node[result] (static) at (-6.5, -4.2) {\texttt{static}\\ schedule};
		\node[result] (dynamic) at (-4.5, -4.2) {\texttt{dynamic}\\ schedule};
		\node[result] (reduce) at (-2.5, -4.2) {Reduction\\ primitives};
		\node[result] (stencil) at (-0.5, -4.2) {Domain\\ decomp.};
		\node[result] (tasks) at (2.5, -4.2) {OpenMP\\ tasks};
		\node[result] (taskgraph) at (4.5, -4.2) {Task graphs\\ runtime};

		% Examples below results
		\node[example] at (-6.5, -5.0) {matrix mult.};
		\node[example] at (-4.5, -5.0) {Julia set};
		\node[example] at (-2.5, -5.0) {dot product};
		\node[example] at (-0.5, -5.0) {heat equation};
		\node[example] at (2.5, -5.0) {quicksort};
		\node[example] at (4.5, -5.0) {graph traversal};

		% Arrows
		\draw[arrow] (root) -- node[above left, font=\tiny] {for-like} (indep);
		\draw[arrow] (root) -- node[above right, font=\tiny] {while-like} (recur);
		\draw[arrow] (indep) -- node[left, xshift=-2pt, font=\tiny] {Yes} (uniform);
		\draw[arrow] (indep) -- node[right, xshift=2pt, font=\tiny] {No} (deptype);
		\draw[arrow] (uniform) -- node[left, font=\tiny] {Yes} (static);
		\draw[arrow] (uniform) -- node[right, font=\tiny] {No} (dynamic);
		\draw[arrow] (deptype) -- node[left, font=\tiny] {Reduce} (reduce);
		\draw[arrow] (deptype) -- node[right, font=\tiny] {Neighbor} (stencil);
		\draw[arrow] (recur) -- node[left, font=\tiny] {Recursive} (tasks);
		\draw[arrow] (recur) -- node[right, font=\tiny] {Irregular} (taskgraph);
	\end{tikzpicture}
	\end{center}
	\vskip0.2em
	\textbf{Key questions to ask:}
	\begin{itemize}
		\item What's the communication pattern? (none / reduction / neighbors / all-to-all)
		\item Is the work per element uniform or variable?
		\item Do I know the work structure at compile time or only at runtime?
	\end{itemize}
\end{frame}

\begin{frame}{\LARGE Beyond Basic Patterns}
	\vspace{-1em}
	\footnotesize
	\begin{columns}[T]
		\begin{column}{0.5\textwidth}
			...
			\begin{tikzpicture}[scale=0.5]
				...
				\node[fill=red!20] at (0.75, 2.25) {\tiny P0};
				\node[fill=blue!20] at (2.25, 2.25) {\tiny P1};
				\node[fill=green!20] at (0.75, 0.75) {\tiny P2};
				\node[fill=yellow!20] at (2.25, 0.75) {\tiny P3};
				% Arrow
				\node at (1.5, -0.5) {\tiny Split space};
			\end{tikzpicture}
			\end{center}
			\vspace{-1.5em}
			\begin{itemize}
				\item Split physical space among processes
				\item Each owns a subdomain
				\item Exchange boundaries (halo/ghost cells)
				\item \textit{PDEs, fluid dynamics, weather}
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{Model Coupling}
			\begin{center}
			\begin{tikzpicture}[scale=0.5]
				\draw[thick, fill=blue!20, rounded corners] (0, 2) rectangle (2, 3);
				\node at (1, 2.5) {\tiny Ocean};
				\draw[thick, fill=cyan!20, rounded corners] (0, 0) rectangle (2, 1);
				\node at (1, 0.5) {\tiny Atmos.};
				\draw[<->, thick] (1, 1.1) -- (1, 1.9);
				\node[right, font=\tiny] at (1.1, 1.5) {exchange};
			\end{tikzpicture}
			\end{center}
			\begin{itemize}
				\item Different physics/codes
				\item Exchange data at interfaces
				\item Often different time scales
				\item \textit{Climate, multi-physics}
			\end{itemize}
		\end{column}
	\end{columns}
	\vskip1em
	\begin{columns}[T]
		\begin{column}{0.5\textwidth}
			\textbf{Ensemble Runs}
			\begin{itemize}
				\item Same code, different inputs
				\item Embarrassingly parallel at job level
				\item \textit{UQ, ML hyperparameters, Monte Carlo}
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\textbf{Particle Methods}
			\begin{itemize}
				\item Distribute particles/agents
				\item Dynamic load balancing
				\item \textit{N-body, molecular dynamics}
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{}
	...
	\vskip1.5em
	{\Large Scaling Exercise with SLURM}
	\vskip2em
	{\large \href{https://ood.dcsr.unil.ch/}{\beamergotobutton{Open OnDemand}}}
	\vfill
\end{frame}
...
\begin{frame}{Additional Resources}
    ...
    \begin{itemize}
        \footnotesize
        \item SLURM Official Documentation: \url{https://slurm.schedmd.com/}
    ...
    \begin{itemize}
        \footnotesize
        \item OpenMP Tutorial: \url{https://www.openmp.org/resources/tutorials-articles/}
        \item MPI Tutorial: \url{https://mpitutorial.com/}
    \end{itemize}
    \vskip0.8em
    \textbf{Tools:}
    \begin{itemize}
...
\begin{frame}{Questions?}
    ...
    \begin{center}
        ...
        \vskip1em
        \normalsize
        \textbf{Contact Information and Cluster Support:}\\
        {\href{mailto:helpdesk@unil.ch?subject={[}DCSR{]} Your Topic}{helpdesk@unil.ch}}\\
        with subject \emph{``[DCSR] Your Topic''}
    \end{center}
\end{frame}
