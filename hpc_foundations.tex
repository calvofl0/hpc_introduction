...
\begin{frame}{What is Parallel Computing?}
    \begin{itemize}
        ...
        \begin{itemize}
            \item Break down a problem into smaller, independent parts
            \item Execute these parts simultaneously on different processing units
            \item Combine results to get the final solution
        \end{itemize}
        \item \textbf{Why Parallel Computing?}
        \begin{itemize}
            \item Overcome physical limitations of single processors
            \item Solve problems that would take too long sequentially
            \item Handle larger datasets and more complex computations
            \item Improve computational efficiency and throughput
        \end{itemize}
        \item \textbf{Range of Applications:}
        \begin{itemize}
            \item From multi-core laptops to massive supercomputers
            \item From simple multi-threaded programs to distributed systems
            \item The foundation of modern High-Performance Computing
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Parallelism vs Concurrency}
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            ...
            \vskip1em
            \textbf{Example: Cooking and talking on phone}
            \begin{tikzpicture}[scale=0.85]
                % Time axis
                \draw[->, thick] (0,0) -- (6,0) node[right] {Time};
                
                % Cooking task
                \node[left] at (0, 1.5) {\tiny Cooking:};
                \draw[thick, fill=orange!40] (0,1.2) rectangle (1.8,1.8);
                \node at (0.9, 1.5) {\tiny Chop};
                \draw[thick, fill=orange!40] (2.2,1.2) rectangle (3.6,1.8);
                \node at (2.9, 1.5) {\tiny Stir};
                \draw[thick, fill=orange!40] (4.0,1.2) rectangle (5.5,1.8);
                \node at (4.75, 1.5) {\tiny Serve};
                
                % Phone task
                \node[left] at (0, 0.7) {\tiny Phone:};
                \draw[thick, fill=blue!40] (1.8,0.4) rectangle (2.2,1.0);
                \node at (2.0, 0.7) {\tiny C};
                \draw[thick, fill=blue!40] (3.6,0.4) rectangle (4.0,1.0);
                \node at (3.8, 0.7) {\tiny C};
