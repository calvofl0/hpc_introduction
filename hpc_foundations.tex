    ...
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            ...
            \begin{tikzpicture}[scale=0.7]
                ...
                % Shared memory
                \draw[thick, fill=blue!20] (-0.3,0.5) rectangle (4.1,1.3);
...
\begin{frame}{Compute Nodes in an HPC Cluster}
    \begin{tikzpicture}[scale=0.9]
        % SLURM managed box
        \draw[thick, dashed, rounded corners] (-0.5,1.8) rectangle (10,6.5);
        \node at (5, 6.2) {\textbf{SLURM managed}};
        
        % Login node
        \draw[thick, fill=yellow!20, rounded corners] (0,4.5) rectangle (2,5.5);
        \node at (1, 5) {Login Node};
        ...
        % Arrows from login to compute
        \draw[->, thick] (1, 4.5) -- (1, 4);
        \draw[->, thick] (1.5, 4.5) -- (3.5, 4);
        \draw[->, thick] (1.7, 4.5) -- (6, 4);
        \draw[->, thick] (1.9, 4.5) -- (8.5, 4);
        ...
        \node at (11, 5) {\small Users SSH here};
        \draw[->, thick] (10.5, 5) -- (2, 5);
    \end{tikzpicture}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{tikzpicture}[scale=0.8]
                ...
                \node[left] at (0,1.75) {\tiny A[0:3], B[0:3]};
    ...
    c[i] = a[i] + b[i];
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{tikzpicture}[scale=0.75]
                % Process
                \draw[thick, rounded corners, fill=yellow!10] (0,0) rectangle (5.5,5);
                \node at (2.75, 4.6) {\textbf{Process}};
                
                % Threads
                \draw[thick, fill=green!20] (0.5,3) rectangle (1.8,4.3);
                \node at (1.15, 3.65) {\tiny Thread 1};
                
                \draw[thick, fill=green!20] (2.1,3) rectangle (3.4,4.3);
                \node at (2.75, 3.65) {\tiny Thread 2};
                
                \draw[thick, fill=green!20] (3.7,3) rectangle (5,4.3);
                \node at (4.35, 3.65) {\tiny Thread 3};
                
                % Shared memory
                \draw[thick, fill=blue!20] (0.5,0.5) rectangle (5,2.5);
                \node at (2.75, 1.5) {Shared Memory};
                
                % Arrows
                \draw[<->, thick] (1.15, 3) -- (1.15, 2.5);
                \draw[<->, thick] (2.75, 3) -- (2.75, 2.5);
                \draw[<->, thick] (4.35, 3) -- (4.35, 2.5);
            \end{tikzpicture}
            \vskip0.5em
...
\begin{frame}{Performance Metrics: Speedup}
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            \textbf{Speedup:}
            \[
            ...
            \begin{itemize}
                ...
                \item $T_{\text{parallel}}(n)$ = time with $n$ processors
                \item Ideal speedup: $S(n) = n$ (linear)
            \end{itemize}
            \vskip1em
            ...
            \]
            \begin{itemize}
                \item Measures how well processors are utilized
                \item Perfect efficiency: 100\%
                \item Typical goal: $>$80\%
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \textbf{Example:}
            \begin{itemize}
                ...
                \item Speedup: $S(4) = 100/30 = 3.33$
                \item Efficiency: $E(4) = 3.33/4 = 83\%$
            \end{itemize}
            \vskip1em
            \textbf{Why not 4× speedup?}
            \begin{itemize}
                \item Serial portions of code
...
\begin{frame}{Strong vs Weak Scaling}
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                ...
                \item Each processor gets less work
                \item Measures time to solution
            \end{itemize}
            \vskip0.5em
            \textbf{Example:}
            \begin{itemize}
                \item Problem: Process 1,000,000 items
                \item 1 core: 1,000,000 items/core
                \item 10 cores: 100,000 items/core
                \item 100 cores: 10,000 items/core
            \end{itemize}
            \vskip0.5em
            \textbf{Goal:} Linear speedup
            \[
            T(n) = \frac{T(1)}{n}
            \]
        \end{column}
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                ...
                \item Increase processors and total problem size
                \item Each processor keeps same workload
                \item Measures throughput
            \end{itemize}
            \vskip0.5em
            \textbf{Example:}
            \begin{itemize}
                \item 1 core: 100,000 items
                \item 10 cores: 1,000,000 items (100k each)
                \item 100 cores: 10,000,000 items (100k each)
            \end{itemize}
            \vskip0.5em
            \textbf{Goal:} Constant time
            \[
            T(n) = T(1)
            \]
        \end{column}
    \end{columns}
    \vskip0.5em
    \textbf{Use cases:} Strong scaling for fixed problems; weak scaling for larger problems
\end{frame}
...
\begin{frame}{Deriving Amdahl's Law}
    ...
    \begin{itemize}
        \item Fraction $p$ that can be parallelized
        \item Fraction $(1-p)$ that must run serially
        \item Total serial execution time normalized to 1
    \end{itemize}
    \vskip1em
    \textbf{With $n$ processors:}
    \begin{itemize}
        \item Serial part takes: $(1-p)$ time (unchanged)
        \item Parallel part takes: $\frac{p}{n}$ time (divided by $n$)
        \item Total parallel time: $T_{\text{parallel}}(n) = (1-p) + \frac{p}{n}$
    \end{itemize}
    \vskip1em
    \textbf{Speedup:}
    \[
    ...
    \]
    \vskip1em
    \textbf{Maximum speedup} (as $n \to \infty$):
    \[
    S_{\text{max}} = \frac{1}{1-p}
    \]
    Example: If only 90\% is parallel, max speedup = 10× (regardless of processors!)
\end{frame}
...
\begin{frame}{Resource Allocation Strategies}
    ...
    \begin{tabular}{p{3cm}p{4cm}p{4.5cm}}
        \toprule
        \textbf{Job Type} & \textbf{SLURM Configuration} & \textbf{Use Case} \\
        \midrule
        Serial & \texttt{--ntasks=1} \newline \texttt{--cpus-per-task=1} & Single-threaded code, simple scripts, many independent runs \\
        \midrule
        Multi-threaded & \texttt{--ntasks=1} \newline \texttt{--cpus-per-task=N} & OpenMP/threads, shared memory code, single node sufficient \\
        \midrule
        MPI & \texttt{--ntasks=N} \newline \texttt{--cpus-per-task=1} & Distributed memory, large scale problems, multiple nodes needed \\
        \midrule
        Hybrid MPI+OpenMP & \texttt{--ntasks=M} \newline \texttt{--cpus-per-task=N} & Both paradigms, very large problems, reduced MPI overhead \\
        \bottomrule
    \end{tabular}
\end{frame}
