...
\begin{frame}{Common Parallel Patterns}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{tikzpicture}[scale=0.55]
				% Stages
				\draw[fill=blue!20] (0, 0.8) rectangle (0.8, 1.3);
				\node at (0.4, 1.05) {\tiny S1};
				\draw[fill=yellow!20] (1.2, 0.8) rectangle (2.0, 1.3);
				\node at (1.6, 1.05) {\tiny S2};
				\draw[fill=green!20] (2.4, 0.8) rectangle (3.2, 1.3);
				\node at (2.8, 1.05) {\tiny S3};
				% Arrows
				\draw[->, thick] (0.85, 1.05) -- (1.15, 1.05);
				\draw[->, thick] (2.05, 1.05) -- (2.35, 1.05);
				% Data items
				\node at (0.4, 0.4) {\tiny a,b,c};
				\node at (1.6, 0.4) {\tiny a,b};
				\node at (2.8, 0.4) {\tiny a};
			\end{tikzpicture}
			\\[0.2em]
			\scriptsize Data flows through stages\\
...
\begin{frame}{Choosing Your Parallelization Strategy}
	...
	\begin{center}
	\begin{tikzpicture}[
		decision/.style={diamond, draw, fill=blue!10, text width=2.2cm, align=center, inner sep=1pt, font=\tiny},
		result/.style={rectangle, draw, fill=green!20, text width=1.8cm, align=center, rounded corners, font=\tiny},
		arrow/.style={->, thick, >=stealth},
		scale=0.9
	]
		% Root
		\node[decision] (root) at (0, 0) {Simple loop structure?};

		% Left branch - Yes (loops)
		\node[decision] (indep) at (-3.5, -1.5) {Iterations independent?};

		% Right branch - No (not loops)
		\node[decision] (recur) at (3.5, -1.5) {Recursive or irregular?};

		% Left-left: independent iterations
		\node[decision] (uniform) at (-5.5, -3) {Uniform work per iteration?};

		% Left-right: dependencies
		\node[decision] (deptype) at (-1.5, -3) {Dependency type?};

		% Results
		\node[result] (static) at (-6.5, -4.5) {\texttt{static}\\ schedule};
		\node[result] (dynamic) at (-4.5, -4.5) {\texttt{dynamic}\\ schedule};
		\node[result] (reduce) at (-2.5, -4.5) {Reduction\\ primitives};
		\node[result] (stencil) at (-0.5, -4.5) {Domain\\ decomp.};
		\node[result] (tasks) at (2.5, -4.5) {OpenMP\\ tasks};
		\node[result] (taskgraph) at (4.5, -4.5) {Task graphs\\ runtime};

		% Arrows
		\draw[arrow] (root) -- node[above left, font=\tiny] {Yes} (indep);
		\draw[arrow] (root) -- node[above right, font=\tiny] {No} (recur);
		...
		\draw[arrow] (deptype) -- node[right, font=\tiny] {Stencil} (stencil);
